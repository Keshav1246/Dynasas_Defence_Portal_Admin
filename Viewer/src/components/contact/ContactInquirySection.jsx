...
const Toast = ({ message, type, onClose }) => (
  <motion.div
    ...
    className={`fixed bottom-8 right-8 z-50 flex items-center gap-3 px-6 py-4 rounded-lg shadow-2xl border ${type === 'success'
        ? 'bg-brand-black border-green-500/30 text-green-500'
        : 'bg-brand-black border-red-500/30 text-red-500'
      }`}
  >
...
const ContactInquirySection = ({ data, forwardRef }) => {
  ...
  return (
    <section ref={forwardRef} className="py-24 bg-brand-black relative z-10 border-t border-brand-white/10 overflow-hidden">
      <div className="container mx-auto px-6 max-w-[1400px]">
        <div className="flex justify-center">

          {/* CENTERED: INQUIRY FORM */}
          <div className="w-full max-w-4xl border border-brand-white/10 bg-brand-dark p-8 md:p-12 lg:p-16 relative overflow-hidden group rounded-sm shadow-2xl">
            {/* Ambient Background */}
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className={`w-full bg-brand-black border ${errors.fullName ? 'border-red-500/50' : 'border-brand-white/10'} text-brand-white px-4 py-3 focus:outline-none focus:border-brand-primary focus:bg-[rgba(255,106,0,0.02)] transition-all`}
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-brand-black border border-brand-white/10 text-brand-white px-4 py-3 focus:outline-none focus:border-brand-primary focus:bg-[rgba(255,106,0,0.02)] transition-all"
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className={`w-full bg-brand-black border ${errors.email ? 'border-red-500/50' : 'border-brand-white/10'} text-brand-white px-4 py-3 focus:outline-none focus:border-brand-primary focus:bg-[rgba(255,106,0,0.02)] transition-all`}
                    />
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className={`w-full bg-brand-black border ${errors.inquiryType ? 'border-red-500/50' : 'border-brand-white/10'} text-brand-white px-4 py-3 focus:outline-none focus:border-brand-primary focus:bg-[rgba(255,106,0,0.02)] transition-all appearance-none`}
                  ...
                  >
                    ...
                    {INQUIRY_TYPES.map((type, idx) => (
                      <option key={idx} value={type} className="bg-brand-black text-brand-white">{type}</option>
                    ))}
                  </select>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className={`w-full bg-brand-black border ${errors.message ? 'border-red-500/50' : 'border-brand-white/10'} text-brand-white px-4 py-3 focus:outline-none focus:border-brand-primary focus:bg-[rgba(255,106,0,0.02)] transition-all resize-y`}
                  ></textarea>
                  {errors.message && <span className="text-xs text-red-500">{errors.message}</span>}
                </div>

                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-4 border-t border-brand-white/10">
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className="w-full sm:w-auto inline-flex items-center justify-center gap-3 bg-brand-primary text-brand-black px-10 py-4 font-heading font-bold uppercase tracking-widest text-sm hover:bg-brand-primary/90 hover:shadow-[0_0_20px_rgba(255,106,0,0.4)] transition-all duration-300 disabled:opacity-70 disabled:cursor-not-allowed"
                  >
